...
def load_frame_ids2(keys):
    ...
    for di in keys:
        ...
        for frame_id in index_20:
            file_name = str(frame_id) + '.jpg'
            frame_path = os.path.join(dir_path, di, file_name).encode('utf-8') # xxx/tumblr_232snw/1.jpg
...
def generate_tsv(gpu_id, prototxt, weights, image_dict, out_file):
            ...
            with open(outfile, 'ab') as tsvfile:
            	...
            	for im_file, image_id in image_ids:
                    if image_id in missing:
                        print(im_file)
                        print('Start to extract frame_%s for video[%s]!' % (image_id, im_file.encode('utf-8').split('/')[-2]))
...
if __name__ == '__main__':
    ...
    # Total videos need to be processed
    import cPickle
    import os
    # keys = cPickle.load(open('videos_extracted_cPickle.pkl', 'rb'))
    keys = os.listdir('/home/chenxh/YSJ/gif_frames')

    # Videos already processed
    video_finished = [file.split('.')[0] for file in os.listdir('/home/chenxh/YSJ/video_tsv')]

    # Videos remained
    keys = list(set(keys) - set(video_finished))
    keys = [key.encode('utf-8') for key in keys]
    num_videos = len(keys)
    print('### Number of videos remained to be processed: ', num_videos)


    key0 = keys[0: int(1*num_videos / NUM_GPUS)]
    key1 = keys[int(1*num_videos / NUM_GPUS) : int(2*num_videos / NUM_GPUS)]
    key2 = keys[int(2*num_videos / NUM_GPUS) : int(3*num_videos / NUM_GPUS)]
    key3 = keys[int(3*num_videos / NUM_GPUS) : int(4*num_videos / NUM_GPUS)]
    key4 = keys[int(4*num_videos / NUM_GPUS) : ]
    # key5 = keys[int(5*num_videos / 8) : int(6*num_videos / 8)]
    # key6 = keys[int(6*num_videos / 8) : int(7*num_videos / 8)]
    # key7 = keys[int(7*num_videos / 8):]
    key_list = [key0, key1, key2, key3, key4]

    dict_list = []
    for i in range(NUM_GPUS):
        dict_i = load_frame_ids2(key_list[i])
        print('Num keys[i]: ', len(dict_i))
    ...
    for i, gpu in enumerate(gpus):
        p = Process(target=generate_tsv, args=(gpu, args.prototxt, args.caffemodel, dict_list[i], out_file))
        p.daemon = True
        p.start()
        procs.append(p)
    for p in procs:
