...
class VideoQANew(Dataset):
    ...
    def __init__(self, ques_type, app_video_feats, mot_id_to_index, reg_id_to_index, questions, vocab_size):
        self.ques_type = ques_type
        self.app_video_feats = app_video_feats # Subsampled already
        self.mot_id_to_index = mot_id_to_index
        # self.app_id_to_index = app_id_to_index
        self.reg_id_to_index = reg_id_to_index # Root directory for faster R-CNN features.
    ...
    def __getitem__(self, i):
        ...
        # appearance_index = self.app_id_to_index[video_id]
        region_index = self.reg_id_to_index[video_name]

        # app_video_feats = []
        # with h5py.File(os.path.join('./data/' , self.ques_type + '_appearance_feat.h5'), 'r') as a_motion:
        #     app_video_feats = a_motion['resnet_features'][appearance_index]       # (8, 16, 2048)
            # app_video_feats.append(video_feats[:, 0, :].unsqueeze(1))
            # app_video_feats.append(video_feats[:, -1, :].unsqueeze(1))
            # app_video_feats = torch.cat(app_video_feats, 1)
        ...
        with h5py.File(os.path.join('./data/region_32_20.h5'), 'r') as f_region:
            region_feats = f_region['region_features'][region_index]                 # (32, 20, 2048)
            region_boxes = f_region['region_boxes'][region_index]                    # (32, 20, 4)

        # region_feats = np.zeros(200)
        # region_boxes = np.zeros(200)
        ...
        if self.ques_type in ['action', 'transition']:
            ...
            for i in range(1, 6):
                ...
                na = 'a' + str(i)
                candidates_answers = candidates_answers + ',' + ques[na]
            ...
            return torch.from_numpy(region_feats), torch.from_numpy(region_boxes), \
                   ...
                   video_name, video_id, question, candidates_answers
...
if __name__ == '__main__':
    ...
    # Execute only once to subsample video frames up to 16 frames, and save the new hdf5 fi    # old_file_path = '/data4/pengliang/HGA/dataset/tgif/features/TGIF_RESNET_pool5.hdf5'
    subsample_path = '/data4/pengliang/HGA/dataset/tgif/features/TGIF_RESNET_16.hdf5'
    # subsample_frames(subsample_path, get_video_feats(old_file_path))le in disk:
